...
async function main() {
    ...
    console.log(`   ConditionId: ${MARKET_STATE.conditionId}`);
    ...
    const yesCollectionId = await publicClient.readContract({
        ...
        args: [PARENT_COLLECTION_ID, MARKET_STATE.conditionId, 1n], // YES = indexSet 1
    });
    ...
    const noCollectionId = await publicClient.readContract({
        ...
        abi: CTF_ABI,
        functionName: 'getCollectionId',
        args: [PARENT_COLLECTION_ID, MARKET_STATE.conditionId, 2n], // NO = indexSet 2
    });
